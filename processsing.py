...
class proc(object):
    ...
    def __init__(
        ...
        if EASYPROCESS_USE_TEMP_FILES:
            print("EASYPROCESS_USE_TEMP_FILES=%s", EASYPROCESS_USE_TEMP_FILES)
            # '0'->false, '1'->true
        ...
        # print('param: "%s" ', self.cmd_param)
        print("command: %s", self.cmd)
        # print('joined command: %s', self.cmd_as_string)
    ...
    def start(self) -> "EasyProcess":
        ...
        try:
            ...
            print("OSError exception: %s", oserror)
            self.oserror = oserror

        self.is_started = True
        print("process was started (pid=%s)", self.pid)
        return self
    ...
    def _wait4process(self, timeout=None):
        ...
        if self.use_temp_files:
            ...
            try:
                ...
                print("timeout")
                return
        print("process has ended, return code=%s", self.return_code)
    ...
    def sendstop(self) -> "EasyProcess":
        ...
        print('stopping process (pid=%s cmd="%s")', self.pid, self.cmd)
        if self.popen:
            if self.is_alive():
                print("process is active -> calling kill()")
                self.popen.kill()

                # signame = "SIGKILL" if kill else "SIGTERM"
                # print("process is active -> sending " + signame)
                ...
                # except OSError as oserror:
                #     print("exception in terminate:%s", oserror)

            else:
                print("process was already stopped")
        else:
            print("process was not started")
